...
def _round24(x: float) -> float:
    """Round to 25 decimal places (floating precision will cap), keep float for downstream math."""
    return round(float(x), 24)

def _be_occupancy_per_state(E: float, mu: float, T: float) -> float:
...
def find_mu_be(energies_sorted, degeneracy_map, N_target, T):
    """
    ...
    mu_low = E_min - 100.0 * KB * T  # far below -> small N
    mu_high = E_min - 1e-30          # just below ground -> huge N

    def N_of_mu(mu):
        total = 0.0
        for E in energies_sorted:
            g = degeneracy_map.get(E, 1)
            occ = _be_occupancy_per_state(E, mu, T)
            total += g * occ
...
def main():
    ...
    print()
    print(f"\nTotal energy after equilibration (J): {ens.system.E_total}")
    print(f"Average energy per particle (J): {ens.system.E_total/N:.6e}")
    ...
    energy = sorted(avg_occ.keys())  # physical energies (rounded)
    ...
    # --- Numerical μ solving to match total N (Option A) ---
    # Build degeneracy list aligned with "energy"
    degen_list = [ens.system.degeneracy.get(e, 1) for e in energy]
    degeneracy_map = ens.system.degeneracy

    # Find μ so that N_theory(μ) = N
    mu = find_mu_be(energy, degeneracy_map, N, T)
    print(f"\nComputed chemical potential μ = {mu:.6e} J")

    # Comparison with Bose–Einstein statistics using found μ
    be_E = []
    be_f_of_E = []
    for e_val in energy:
        be_E.append(e_val)
        f_of_E = _be_occupancy_per_state(e_val, mu, T)
        be_f_of_E.append(f_of_E)

    # Check if we have valid BE values
    if max(be_f_of_E) > 0:
        # Normalize BE distribution to match simulation scale (same approach as before)
        max_sim = max(avg_per_energy)
        max_be = max(be_f_of_E)
        be_f_of_E_scaled = [f * (max_sim / max_be) for f in be_f_of_E]

        plt.plot(be_E, be_f_of_E_scaled, label='Bose-Einstein (μ solved)', 
